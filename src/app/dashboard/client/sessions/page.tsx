...
import { getMySessions, getApprovedExperts } from "@/lib/data/queries";
...
export default function ClientSessionsPage() {
  ...
  useEffect(() => {
    Promise.all([getMySessions(), getApprovedExperts()])
      .then(([mySessions, approvedExperts]) => {
            ...
            <div className="flex justify-between items-center gap-3 pt-6 border-t border-[var(--border)] flex-wrap">
              {session.paymentStatus === "unpaid" ? (
                <button
                  ...
                  className="px-4 py-2 rounded-full text-[10px] font-bold uppercase tracking-widest bg-orange-400/10 text-orange-400 border border-orange-400/30 hover:bg-orange-400/20 transition-all flex items-center gap-2"
                >
