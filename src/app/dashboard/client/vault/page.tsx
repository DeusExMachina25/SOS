...
export default function ClientVaultPage() {
  ...
  const handleUploadMockFile = () => {
    const name = prompt("Enter file name to encrypt and upload:", "Architectural_Plan_v3.pdf");
    if (!name) return;
  ...
  const filteredFiles = files.filter(file => {
    ...
    if (activeTab === "My Uploads") {
      return matchesSearch && file.expert === "Admin"; // Mocking admin as user uploads
    }
    if (activeTab === "Shared with Me") {
      return matchesSearch && file.expert !== "Admin";
    }
