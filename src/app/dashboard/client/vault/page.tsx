...
export default function ClientVaultPage() {
  ...
  const handlePickFile = () => {
    if (!selectedSessionId) {
      setError("Book a session before uploading — files are attached to a session.");
      return;
    }
    ...
    <div className="w-full relative min-h-screen flex flex-col pt-6 pb-24">
      ...
      >
        ...
        <p className="text-sm text-[var(--text-muted)] font-inter mb-6">
          {sessions.length > 0 ? "Click to choose a file for the selected session." : "Book a session first — files attach to a session."}
        </p>
