...
import { getMySessions, getMyVaultFiles, getMyProfile } from "@/lib/data/queries";
import type { Session, VaultFile } from "@/lib/data/types";
import { formatFileSize, formatSessionDate } from "@/lib/data/format";
...
export default function ExpertDashboard() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const [mySessions, myFiles, profile] = await Promise.all([
          getMySessions(),
          getMyVaultFiles(),
          getMyProfile(),
        ]);
  ...
  // RLS already scopes `sessions` to this expert.
              ...
              {files.slice(0, 2).map((file) => (
                <div key={file.id} className="p-4 bg-[var(--bg-surface-2)] border border-[var(--border)] rounded-2xl flex items-center justify-between">
                  <div className="min-w-0">
                    ...
                    <p className="text-[9px] font-mono-sos text-[var(--text-faint)] uppercase tracking-wider mt-1">{formatFileSize(file.sizeBytes)} • {formatSessionDate(file.createdAt)}</p>
                  </div>
