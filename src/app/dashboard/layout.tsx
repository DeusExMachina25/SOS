...
export default function DashboardLayout({
  ...
  useEffect(() => {
    ...
    getMyExpertProfile()
      .then((profile) => setUpiIdInput(profile?.upiId ?? ""))
  ...
  const handleSaveUpi = async () => {
    ...
    setUpiSaving(true);
    try {
      await setMyUpiId(upiIdInput.trim());
      setUpiSaved(true);
  ...
  const navItems = [
    ...
    ...(role === "expert"
      ? [{ name: "Availability", icon: CalendarClock, path: `${basePath}/availability`, colorClass: "text-purple-400 group-hover/nav:text-purple-400" }]
      : []),
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                ...
                <div className="space-y-8">
                  ...
                  {role === "expert" && (
                    <div>
                      <label className="block text-xs font-mono-sos text-[var(--text-faint)] mb-3 tracking-widest flex items-center gap-2"><Smartphone size={14} className="text-emerald-400"/> PAYOUT UPI ID</label>
                      <div className="flex gap-3">
                        ...
                        <button
                          ...
                          disabled={upiSaving}
                          className="px-6 rounded-2xl bg-[var(--bg-surface-2)] border border-[var(--border-strong)] text-[var(--text-primary)] text-xs font-bold hover:border-emerald-400 transition-all disabled:opacity-50 flex items-center gap-2"
                        >
                          {upiSaved ? <Check size={14} className="text-emerald-400" /> : upiSaving ? "..." : "Save"}
                        </button>
                      </div>
                      {upiError && <p className="text-[10px] text-red-400 mt-2">{upiError}</p>}
