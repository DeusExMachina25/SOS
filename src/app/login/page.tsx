...
export default function LoginPage() {
  ...
  // Dev-only shortcuts (seeded accounts) are gated on the development build.
  const isDev = process.env.NODE_ENV === "development";

  // After any successful sign-in, route to the dashboard for the user's role.
  const routeByRole = async () => {
    ...
    const { data: profile } = await supabase
      ...
      .single();
    router.push(
      profile?.role === "client" ? "/dashboard/client" : "/dashboard/expert"
    );
  };
    ...
    if (inputType === "email") {
      ...
      try {
        if (
          isDev &&
          supabase &&
          (cleanedInput.toLowerCase() === "test@sos.com" ||
            cleanedInput.toLowerCase() === "client@sos.com")
        ) {
          // Dev bypass → real seeded client session.
          const { error } = await supabase.auth.signInWithPassword({
            email: "client@sos.com",
            password: "password",
          });
      ...
      try {
        if (isDev && phoneNum === "0000000000") {
          // Dev bypass → show the OTP screen; sign-in happens on verify.
          setTimeout(() => {
            setOtpSent(true);
            setLoading(false);
          }, 1200);
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      if (isDev && supabase && phoneNum === "0000000000" && otp === "123456") {
        // Dev bypass → real seeded client session.
        const { error } = await supabase.auth.signInWithPassword({
          email: "client@sos.com",
          password: "password",
  ...
  // Dev-only: simulate clicking the magic link by signing into the seeded client.
  ...
  const handleExpertLogin = async (e: React.FormEvent) => {
    ...
    try {
      if (
        ...
        supabase &&
        password === "password" &&
        ["test@sos.com", "expert@sos.com", "admin@sos.com"].includes(email)
      ) {
        // Dev bypass → real seeded session (test@sos.com maps to the expert).
        const { error } = await supabase.auth.signInWithPassword({
          email: email === "test@sos.com" ? "expert@sos.com" : email,
          password: "password",
        });
        if (error) throw error;
        await routeByRole();
